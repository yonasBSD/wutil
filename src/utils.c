...
struct network_configuration *generate_network_configuration(int argc,
  ...
  while ((opt = getopt_long(argc, argv, "m:i:n:g:d:s:r:", options, NULL)) !=
    ...
    case 'm':
      ...
      break;
    case 'i':
      if (config->method == UNCHANGED || config->method != MANUAL) {
...
// TODO: properly do it
int configure_ip(char *interface_name, struct network_configuration *config) {
  char ip_rc[256] = "sysrc ";
  strncatf(ip_rc, sizeof(ip_rc), "ifconfig_%s=\"%s", interface_name,
           strstr(interface_name, "wlan") ? "WPA " : "");

  if (config->method == MANUAL)
    strncatf(ip_rc, sizeof(ip_rc), "inet %s netmask %s\"", config->ip,
             config->netmask);
  else
  ...
  if (config->method == MANUAL) {
    char gateway_rc[256];
    snprintf(gateway_rc, sizeof(gateway_rc), "sysrc defaultrouter=\"%s\"",
             config->gateway);
...
int configure_resolvd(struct network_configuration *config) {
